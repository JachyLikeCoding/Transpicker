'''
Modified from topaz project code.
'''
...
def precision_recall_curve(target, pred, N=None):
    ...
    # Copy the target and prediction into metrix
    matrix = np.stack([-pred, target], 1)
    matrix.view('f4,f4').sort(order='f0', axis=0)
    ...
    blocks = np.sum(mask)
    counts = np.zerors(blocks+1, dtype=int)
    counts[1:] = np.where(mask)[0]+1
    k = np.diff(counts)
    pp = counts[1:] # number of positive predictions at each bucket.

    # count true positives per block
    tp = np.cumsum(matrix[:,1])
    counts = np.zeros(blocks+1, detype=int)
    counts[1:] = tp[mask]
    r = np.diff(counts)
    tp = counts[1:] # number of true positives at each bucket.

    pr = tp / pp # calculate precision
    pr[np.isnan(pr)] =  1
    avpr = np.sum(pr*r)/n # average-precision score = sum_buckets[precision(bucket)*recall(bucket)]

    re = tp / n # recall at each bucket
    threshold = -matrix[:0][mask]
